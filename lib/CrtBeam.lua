...
// Triad width in SCREEN pixels. A mask finer than the panel can resolve is
// a grey wash plus moire, so this is in real pixels rather than source ones.
#define TRIAD_PX 3.0
...
vec3 mask(vec2 px, float kind)
{
    ...
    if (kind < 2.5) {
        ...
        float row = floor(px.y / (TRIAD_PX * 2.0));
...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 pc)
{
    ...
    if (beamGlow > 0.0) {
        ...
        vec2 t2 = 4.0 * ps / love_ScreenSize.xy;
        vec3 near = vec3(0.0), far = vec3(0.0);
        for (int i = 0; i < 4; i++) {
            float a = float(i) * (PI * 0.5) + 0.39;
            vec2 d = vec2(cos(a), sin(a));
            near += srgb2lin3(Texel(tex, tc + d * t1).rgb);
            far  += srgb2lin3(Texel(tex, tc + d * t2).rgb);
        }
        near *= 0.25; far *= 0.25;
        lin += (far * HALATION_WEIGHT + near * DIFFUSION_WEIGHT) * beamGlow;
    }
...
CrtBeam.ROLL = { off = 0, ["60hz"] = 60, ["50hz"] = 50, ["40hz"] = 40 }
